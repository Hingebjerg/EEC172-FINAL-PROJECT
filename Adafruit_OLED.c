...
void writeCommand(unsigned char c) {
	...
	unsigned long d = c;
	unsigned long f;
	 MAP_SPIDataPut(GSPI_BASE, d);
	 MAP_SPIDataGet(GSPI_BASE, &f);
...
*/
  ...
  GPIOPinWrite(GPIOA0_BASE, 0x40, 0);	// RESET = RESET_LOW

  for(delay=0; delay<10; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40);	// RESET = RESET_HIGH
